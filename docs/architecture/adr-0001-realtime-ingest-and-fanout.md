...
3. The API replicates each public topic into eight ordered Durable Object fan-out shards and assigns connecting sockets by a hash of the WebSocket handshake key. This keeps the 2,000-client target near 250 sockets per object while every shard receives the same ordered event batches. Private user topics remain on one authorization-isolated object. No global or cross-topic sequence is promised. A sixteen-shard staging trial caused bounded ingest-buffer drops, so that topology is explicitly rejected unless the publish path is redesigned and requalified.
