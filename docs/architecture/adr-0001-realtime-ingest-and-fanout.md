...
3. The API replicates each public topic into sixteen ordered Durable Object fan-out shards and assigns connecting sockets by a hash of the WebSocket handshake key. This keeps the 2,000-client target near 125 sockets per object while every shard receives the same ordered event batches. Private user topics remain on one authorization-isolated object. No global or cross-topic sequence is promised.
